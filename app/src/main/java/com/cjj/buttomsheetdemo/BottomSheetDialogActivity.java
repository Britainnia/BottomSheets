...
public class BottomSheetDialogActivity extends AppCompatActivity {
    ...
    private void showBSDialog() {
        ...
        final ShoppingCarListAdapter andapter = new ShoppingCarListAdapter(this, namelist);
        andapter.setAddClickListener(new ShoppingCarListAdapter.AddClickListener() {
            @Override
            public void onItemClick(int pos) {
                number = namelist.get(pos).getNumber();
                if (number < 30) {
                    namelist.get(pos).setNumber(++number);
                    andapter.notifyItemChanged(pos);
                }

            }
        });
        andapter.setRemoveClickListener(new ShoppingCarListAdapter.RemoveClickListener() {
            @Override
            public void onItemClick(int pos) {
                number = namelist.get(pos).getNumber();
                if (number > 1) {
                    ...
                    namelist.remove(pos);
                    andapter.notifyDataSetChanged();
                }

            }
        });
        recyclerView.setAdapter(andapter);
